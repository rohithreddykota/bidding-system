...
BEGIN
    ...
    BEGIN
        IF (
            (SELECT MAX(BidPrice)
            FROM Bid
            WHERE AuctionID IN (SELECT AuctionID
            FROM inserted)) >= (SELECT MIN(BidPrice)
            FROM inserted)
            OR
            (SELECT MIN(BidPrice)
            FROM inserted) <= (SELECT BuyNowPrice
            FROM Auction
            WHERE AuctionID IN (SELECT AuctionID
            FROM inserted))
        )
        ...
        BEGIN
            INSERT INTO Bid
                (AuctionID, BidPrice, BidTimestamp)
            SELECT AuctionID, BidPrice, BidTimestamp
            FROM inserted;
        END
    END
END;
...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO Auction
            (AdItemID, Title, AuctionDescription, BasePrice, StartDate, EndDate)
        SELECT AdItemID, Title, AuctionDescription, BasePrice, StartDate, EndDate
        FROM inserted;
    END
END;
